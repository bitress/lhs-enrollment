...
<body id="page-top">
                            ...
                            <div class="col-md-12">
                                <div class="card shadow">
                                    <div class="card-header">
                                        <h4><i class="fas fa-user"></i> Faculty List
                                            ...
                                            <tbody>
                                                <?php
                                                    ...
                                                    $sql = "SELECT `faculty_id`, `honorifics`, `fname`, `mname`, `lname`, `title`, `position`, `birthdate`, CURDATE(), TIMESTAMPDIFF(YEAR, birthdate, CURDATE()) as `age`, `email`, `username`, `password`, `userType`
                                                            FROM `tbl_faculty` WHERE `status` = 1 AND userType != 'co_admin'";
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        ...
                                                        <button type="button" value="<?php echo $row["faculty_id"] ?>" class="deleteFacultyBtn btn btn-info btn-sm"><i class="fas fa-archive"></i></button>
                                                    </td>
                                                </tr>
    ...
    <script type="text/javascript">
        //add
        $(document).on('submit', '#save_faculty', function (e) {
            e.preventDefault();
        ...
        $(document).on('click', '.editFacultyBtn', function () {
            ...
            $.ajax({
                type: "GET",
                url: "action/code-faculty.php?faculty_id=" + faculty_id,
                success: function (response) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(res.status == 404) {
                        ...
                        $('#honorifics_v').val(res.data.honorifics);
                        $('#fname_v').val(res.data.fname);
                        $('#mname_v').val(res.data.mname);
                        $('#lname_v').val(res.data.lname);
                        $('#title_v').val(res.data.title);
                        $('#gender_v').val(res.data.gender);
                        $('#birthdate_v').val(res.data.birthdate);
                        $('#position_v').val(res.data.position);
                        $('#username_v').val(res.data.username);
                        $('#password_v').val(res.data.password);
                        $('#email_v').val(res.data.email);
                        $('#userType_v').val(res.data.userType);
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if(response == 'success') {
                                ...
                                Swal.fire({
                                    //title: 'Hello SweetAlert 2!',
                                    text: 'Faculty Record moved to Archive Successfully',
                                    icon: 'success',
                                    timer: '1500',
                                    //confirmButtonText: 'OK',
                                    showConfirmButton: false
